...
> See it work, on a held-out sequence: each tracked object is a car drawn from Retina's WorldState; the **indigo arrow is the real trained model's predicted next move**, rolled out from that one state, and **gray is the actual path** for honest comparison. The straight car is nailed; the curving car shows the honest divergence where the appearance latent earns its keep. Generated by the real model on the Mac Studio (MPS) — [`make_demo_gif.py`](examples/world_model/make_demo_gif.py). Static plot: [`media/rollout.png`](examples/world_model/media/rollout.png).
