...
> **Real match footage → Retina WorldState → a learned dynamics model predicts each player's next run.** A short broadcast clip (Roboflow's MIT-licensed [`sports`](https://github.com/roboflow/sports) sample, originally DFL Bundesliga) goes through a real YOLO detector + tracker, a frozen DINOv2-small appearance encoder, and out as standardized Retina `WorldState`s; the small dynamics transformer trains offline on those sequences and predicts each tracked player's **next ~0.7s** (indigo arrow), with the **actual** run in gray. Honest by design: player motion is stochastic, so on held-out next-step error the learned model only *ties* constant-velocity (7.76 vs 7.79 px) — the win of the appearance latent shows up on the cleaner synthetic ablation above, not on free-running humans. Real pipeline, end to end — [`examples/world_model/soccer/`](examples/world_model/soccer/). The synthetic car rollout (held-out, where the latent earns its keep) lives in [`make_demo_gif.py`](examples/world_model/make_demo_gif.py) · [`media/rollout.png`](examples/world_model/media/rollout.png).
